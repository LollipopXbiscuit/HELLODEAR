...
async def message_counter(update: Update, context: CallbackContext) -> None:
    ...
    if not is_privileged and detect_spam(user_id):
        await update.message.reply_text(
            "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> **Spam Detected!** <tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji>\n\n"
            "You've been temporarily blocked for sending too many messages quickly.\n"
            "<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> **Block Duration:** 12 minutes\n\n"
...
async def guess(update: Update, context: CallbackContext) -> None:
    ...
    if is_user_blocked(user_id):
        ...
        await update.message.reply_text(
            f"<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> **You are temporarily blocked!**\n\n"
    ...
    if user_data:
        ...
        if today_count >= 30:
            await update.message.reply_text(
                ...
                f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You've already married **{today_count}/30** characters today.\n\n"
    ...
    if chat_id not in last_characters:
        await update.message.reply_text('<tg-emoji emoji-id="5102581715000362771">ℹ️</tg-emoji> No character has been summoned yet!\n\nCharacters appear automatically every 100 messages, or admins can use /summon to spawn one manually.')
        return
    ...
    if chat_id in first_correct_guesses and chat_id not in manually_summoned:
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji>️ Already Guessed By Someone.. Try Next Time Bruhh ')
        return

    guess = ' '.join(context.args).lower() if context.args else ''
    
    if "()" in guess or "&" in guess.lower():
        await update.message.reply_text("Nahh You Can't use This Types of words in your guess..<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji>️")
        return
    ...
    if smart_name_match(guess, name_parts):
        ...
        await update.message.reply_text('Please Write Correct Character Name... <tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji>️')
...
async def unmute(update: Update, context: CallbackContext) -> None:
    ...
    if str(sender_id) not in [str(u) for u in sudo_users] and str(sender_id) != str(OWNER_ID):
        await update.message.reply_text("<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> This command is only available to sudo users.")
        return
    ...
    # Try to get user from argument
        ...
        try:
            ...
            await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Please provide a valid user ID or reply to the user's message.")
            return

    if target_id is None:
        await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Reply to a user's message or provide their user ID.\n\nUsage: `/unmute <user_id>`", parse_mode='HTML')
        return
