...
async def lockspawn(client, message):
    ...
    if not character:
        await message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Character with ID `{character_id}` not found!")
        return
    ...
    if existing_lock:
        await message.reply_text(
            f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> **Already Locked!**\n\n"
...
async def unlockspawn(client, message):
    ...
    if not locked_character:
        await message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Character with ID `{character_id}` is not currently locked!")
        return
...
async def lockedspawns(client, message, page=0):
    ...
    if total_pages > 1:
        buttons = []
        if page > 0:
            buttons.append(InlineKeyboardButton("<tg-emoji emoji-id='5102932600943544398'>➡️</tg-emoji> Previous", callback_data=f"lockedspawns:{page-1}"))
...
async def unlockspawn_ptb(update: Update, context: CallbackContext):
    ...
    locked_character = await locked_spawns_collection.find_one({'character_id': character_id})
    if not locked_character:
        await update.message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Character with ID `{character_id}` is not currently locked!")
        return
...
async def broadcast(client, message):
    ...
    if message.reply_to_message:
        ...
        if not broadcast_text:
            await message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Please provide a message to broadcast!")
            return
        is_reply = False
            ...
            try:
                ...
                if (i + 1) % 25 == 0:
                    await status_msg.edit_text(
                        ...
                        f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Failed: {failed_users}"
                    )
            ...
            try:
                ...
                if (i + 1) % 10 == 0:
                    await status_msg.edit_text(
                        ...
                        f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Failed: {failed_groups}"
                    )
...
async def broadcast_ptb(update: Update, context: CallbackContext) -> None:
    ...
    if update.message.reply_to_message:
        ...
        broadcast_text = " ".join(args)
        if not broadcast_text:
            await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Please provide a message to broadcast!")
            return
        is_reply = False
...
async def bonk(client, message):
    ...
    if sender_id != int(OWNER_ID):
        await message.reply_text("<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> This command is only available to the bot owner.")
        return
    ...
    if message.reply_to_message:
        ...
        try:
            ...
            await message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Invalid user ID!")
            return
    else:
        await message.reply_text(
    ...
    if target_id == int(OWNER_ID):
        await message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You can't bonk yourself!")
        return
    ...
    if existing_ban:
        ...
        await message.reply_text(
            ...
            f"<tg-emoji emoji-id='5102857782613248388'>⬅️</tg-emoji> Remaining: {days} days"
        )
        return
...
async def unbonk(client, message):
    ...
    existing_ban = await banned_users_collection.find_one({'user_id': target_id})
    if not existing_ban:
        await message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> This user is not bonked!")
        return
...
async def bonk_ptb(update: Update, context: CallbackContext) -> None:
    ...
    if sender_id != int(OWNER_ID):
        await update.message.reply_text("<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> This command is only available to the bot owner.")
        return
    ...
    if target_id == int(OWNER_ID):
        await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You can't bonk yourself!")
        return
...
async def unbonk_ptb(update: Update, context: CallbackContext) -> None:
    ...
    existing_ban = await banned_users_collection.find_one({'user_id': target_id})
    if not existing_ban:
        await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> This user is not bonked!")
        return
...
async def resetm(client, message):
    ...
    if not user:
        await message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> User ID `{target_id}` not found in database!")
        return
...
async def resetm_ptb(update: Update, context: CallbackContext) -> None:
    ...
    user = await user_collection.find_one({'id': target_id})
    if not user:
        await update.message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> User ID `{target_id}` not found in database!")
        return
