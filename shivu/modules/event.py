...
async def startevent(update: Update, context: CallbackContext) -> None:
    ...
    if str(update.effective_user.id) not in sudo_users:
        await update.message.reply_text('<tg-emoji emoji-id="5102581715000362771">ℹ️</tg-emoji> You do not have permission to use this command.')
        return
    ...
    try:
        ...
        if christmas_count == 0:
            await update.message.reply_text(
                '<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> <b>Cannot start Christmas event!</b>\n\n'
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error starting event: {str(e)}')
...
async def endevent(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not active_event:
            await update.message.reply_text(
                '<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> <b>No active event!</b>\n\n'
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error ending event: {str(e)}')
...
async def eventstatus(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error checking event: {str(e)}')
