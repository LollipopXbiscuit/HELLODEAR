...
def get_format_text(level):
    if level == 1:
        return """Wrong <tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji>️ format...  eg. /upload Img_url muzan-kibutsuji Demon-slayer 5
    ...
    try:
        ...
        try:
            ...
            if level == 1 and rarity_num > 6:
                await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Level 1 uploaders can only upload up to Mythic rank (1-6).')
                return
            if level == 2 and rarity_num > 9:
                await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Level 2 uploaders can only upload up to Zenith rank (1-9).')
                return
...
async def update_card(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not character:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Character with ID #{character_id} not found!')
            return
...
async def summon(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error summoning character: {str(e)}')
...
async def remove_character_from_user(update: Update, context: CallbackContext) -> None:
    ...
    try:
        args = context.args
        if not args or len(args) != 2:
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Incorrect format!\n\nUsage: /remove <character_id> <user_id>\nExample: /remove 123 987654321')
            return
        ...
        try:
            ...
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Invalid user ID format!')
            return

        # Find the character first to show details
        character = await collection.find_one({'id': character_id})
        if not character:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Character with ID #{character_id} not found in database!')
            return
        ...
        if not user:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> User with ID {user_id} not found!')
            return
        ...
        if user_character_count == 0:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> User does not have character #{character_id} ({character["name"]}) in their harem!')
            return
        ...
        if result.modified_count > 0:
            ...
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Failed to remove character from user harem!')
            
    except Exception as e:
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error removing character: {str(e)}')
...
async def find(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not character:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> No character found with ID #{character_id}')
            return
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error finding character: {str(e)}')
...
async def migrate_rarities(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error during migration: {str(e)}')
...
async def adduploader(update: Update, context: CallbackContext) -> None:
    ...
    if str(update.effective_user.id) not in OWNERS:
        await update.message.reply_text('<tg-emoji emoji-id="5102581715000362771">ℹ️</tg-emoji> Only owners can use this command.')
        return
    ...
    try:
        ...
        if not user_id_to_add.isdigit():
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Invalid user ID! Please provide a numeric user ID.')
            return
        ...
        if existing:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> User `{user_id_to_add}` is already an uploader!')
            return
        
        # Check if already sudo user
        if user_id_to_add in sudo_users:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> User `{user_id_to_add}` is already a sudo user (has all permissions)!')
            return
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error adding uploader: {str(e)}')
...
async def removeuploader(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not uploader:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> User `{user_id_to_remove}` is not currently an uploader!')
            return
        ...
        await update.message.reply_text(
            ...
            f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> **Permissions:** Upload access revoked\n'
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error removing uploader: {str(e)}')
...
async def customupload(update: Update, context: CallbackContext) -> None:
    ...
    level = await get_uploader_level(update.effective_user.id)
    if level != 3:
        await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Only level 3 artists (<tg-emoji emoji-id="5102763490901231643">👤</tg-emoji>) can use /customupload command.')
        return
    ...
    try:
        ...
        if not args or len(args) != 4:
            await update.message.reply_text(
                '<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Wrong format!\n\n'
        ...
        try:
            ...
            if slot not in [1, 2, 3]:
                await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Slot must be 1, 2, or 3.')
                return
        except ValueError:
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Slot must be a number (1, 2, or 3).')
            return
        
        # Validate URL
        is_valid, validation_message = validate_url(url)
        if not is_valid:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Invalid URL: {validation_message}')
            return
        ...
        if slot == 2 and not is_video:
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Slot 2 must be a video URL (MP4, MOV, AVI, MKV).')
            return
        if slot in [1, 3] and is_video:
            await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Slots 1 and 3 must be image URLs, not videos.')
            return
        ...
        if not custom_char:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Custom character with ID {char_id} not found.')
            return
        ...
        slot_type = '<tg-emoji emoji-id="5103128992618120782">🤩</tg-emoji> Video' if is_video else '<tg-emoji emoji-id="5102861579364337661">🗑️</tg-emoji> Image'
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error uploading custom slot: {str(e)}')
...
async def customchange(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not custom_char:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Character with ID {char_id} not found.')
            return
        
        if custom_char.get('rarity') != 'Custom':
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> This character is not a Custom character.')
            return
        ...
        if len(args) > 1:
            try:
                ...
                if not slot_data or (isinstance(slot_data, dict) and not slot_data.get('url')):
                    ...
                    await update.message.reply_text(
                        f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Slot {new_slot} ({slot_names.get(new_slot, "Unknown")}) is empty.\n\n'
        ...
        if has_url(slot1_data):
            ...
            slots_display += "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> empty"
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Error: {str(e)}')
...
async def debug_card(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if not char:
            await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Card {card_id} not found.')
            return
        
        msg = f"<tg-emoji emoji-id='5102882435725527517'>📋</tg-emoji> Card Debug Info:\n\n"
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Debug error: {str(e)}')
...
async def migrate_slots(update: Update, context: CallbackContext) -> None:
    ...
    if update.effective_user.id not in sudo_users:
        await update.message.reply_text('<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Admin only command.')
        return
    ...
    try:
        ...
        await update.message.reply_text(f'<tg-emoji emoji-id="5102920111178647010">⚠️</tg-emoji> Migration error: {str(e)}')
