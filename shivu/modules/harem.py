...
async def sorts(update: Update, context: CallbackContext) -> None:
    ...
    if sort_type == 'rarity':
        if len(args) < 2:
            ...
            await update.message.reply_text(
                "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Please specify a rarity!\n\n"
                "<b>Valid rarities:</b>\n" + 
        ...
        if rarity_filter not in valid_rarities:
            await update.message.reply_text(
                f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Invalid rarity '{rarity_filter}'!\n\n"
                "<b>Valid rarities:</b>\n" + 
                "\n".join([f"• <code>{r}</code>" for r in valid_rarities]),
                parse_mode='HTML'
            )
        ...
        if len(args) < 2:
            await update.message.reply_text(
                "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Please specify a character name!\n\n"
        ...
        if not user or not user.get('characters'):
            await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have any characters yet!")
            return
        ...
        if not character_exists:
            await update.message.reply_text(
                f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have any characters named '{character_filter}' in your collection!",
                parse_mode='HTML'
            )
            return
        ...
        await update.message.reply_text(
            "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Invalid command!\n\n"
            "Available options:\n"
...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    if context.args and len(context.args) >= 1:
        if str(requester_id) in [str(u) for u in Config.sudo_users]:
            try:
                ...
                if update.message:
                    await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Invalid user ID!")
                return
        else:
            if update.message:
                await update.message.reply_text("<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> Only admins can view other people's harems!")
            return
    ...
    if not is_sudo_viewing_other:
        if not await check_group_membership(requester_id):
            message_text = (
                "<tg-emoji emoji-id='5102581715000362771'>ℹ️</tg-emoji> <b>Access Restricted</b>\n\n"
...
async def fav(client, message):
    ...
    user = await user_collection.find_one({'id': user_id})
    if not user or not user.get('characters'):
        await message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have any characters yet!")
        return
    
    # Find the character
    character = next((c for c in user['characters'] if c['id'] == character_id), None)
    if not character:
        await message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have character ID `{character_id}` in your collection!", parse_mode=enums.ParseMode.HTML)
        return
    ...
    keyboard = PyroInlineKeyboardMarkup([
        ...
        [PyroInlineKeyboardButton("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Cancel", callback_data="cancel_fav")]
    ])
...
async def fav_callback(client, callback_query):
    ...
    if user_id not in pending_favorites:
        await callback_query.answer("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> No pending favorite found!", show_alert=True)
        return
    ...
    if callback_query.data == "confirm_fav":
        ...
        await callback_query.edit_message_caption(
            caption="<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> <b>Favorite cancelled.</b>",
            parse_mode=enums.ParseMode.HTML
        )
...
async def transfer_harem(update: Update, context: CallbackContext) -> None:
    ...
    if str(user_id) not in [str(u) for u in Config.sudo_users]:
        await update.message.reply_text('<tg-emoji emoji-id="5102581715000362771">ℹ️</tg-emoji> This command is only available to bot administrators.')
        return
    ...
    try:
        ...
        await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Invalid user IDs! Please provide valid numeric user IDs.")
        return
    
    if old_user_id == new_user_id:
        await update.message.reply_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Old and new user IDs cannot be the same!")
        return
    ...
    if not old_user or not old_user.get('characters'):
        await update.message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> User {old_user_id} has no characters to transfer!")
        return
...
async def fav_ptb(update: Update, context: CallbackContext):
    ...
    character = next((c for c in user['characters'] if c['id'] == character_id), None)
    if not character:
        await update.message.reply_text(f"<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have character ID `{character_id}` in your collection!", parse_mode='HTML')
        return
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> Cancel", callback_data="cancel_fav")]
    ])
...
async def fav_callback_ptb(update: Update, context: CallbackContext):
    ...
    if user_id not in pending_favorites:
        await query.answer("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> No pending favorite found!", show_alert=True)
        return
    ...
    if query.data == "confirm_fav":
        ...
        try:
            ...
            await query.message.edit_text("<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> <b>Favorite cancelled.</b>", parse_mode='HTML')
...
async def all_rarities(update: Update, context: CallbackContext) -> None:
    ...
    if not user:
        await update.message.reply_text(
            "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You haven't started collecting yet!\n\n"
    ...
    if not user_characters:
        await update.message.reply_text(
            "<tg-emoji emoji-id='5102920111178647010'>⚠️</tg-emoji> You don't have any characters yet!\n\n"
            "Characters appear every 100 messages. Use /marry to collect them!",
            parse_mode='HTML'
        )
        return
